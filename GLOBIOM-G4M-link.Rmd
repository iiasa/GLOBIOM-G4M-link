...
## Perform the initial GLOBIOM scenarios run

```{r}
cluster_nr <- run_GLOBIOM_scenarios()
...
run_postproc_initial(WD_GLOBIOM, cluster_nr)
...
cluster_nr <- run_initial_downscaling()
...
merge_and_transfer(cluster_nr)
```

## Preliminary: Run baseline G4M scenarios - needs data from the intial downscaling

```{r}
if (!file_exists(path(PATH_FOR_G4M,str_glue("{GDX_OUTPUT_NAME}_output_{PROJECT}_{DATE_LABEL}.gdx"))))
  stop("File for G4M run not found! Please call the intial downscaling before the G4M run")

# Set wd
wd <- path_wd(WD_G4M)

# Generate csv input files for G4M 
gdx_to_csv(wd)
```
...
# Define baseline scenario switch
baseline_run_g4m <- TRUE

# Run G4M - baseline scenarios
run_G4M()
...
## Preliminary: Run G4M scenarios - needs data from the intial downscaling and G4M baseline scenarios

```{r}
allfiles <- dir_ls(path(CD, "G4M", "out", str_glue("{PROJECT}_{DATE_LABEL}"), "baseline"))
...
# Baseline scenario switch
baseline_run_g4m <- FALSE
...
## Preliminary: Compile G4M data and produce the report for GLOBIOM  - needs data from the G4M run

```{r}
# Compile data for GLOBIOM
compile_g4m_data(wd)
```

## Preliminary: Perform the final post-processing - needs data from the G4M run
