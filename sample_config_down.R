...
G00_OUTPUT_FILE = "d1_out.g00" # host-side, will be remapped with EXPERIMENT and cluster/job numbers to avoid name collisions when transferring back to the submit machine.
GET_G00_OUTPUT = FALSE
GDX_OUTPUT_DIR = "gdx"# path for gdx files output
GDX_OUTPUT_FILE = "downscaled.gdx" # as produced on the host-side by gdx= GAMS parameter or execute_unload, will be remapped with EXPERIMENT and cluster/job numbers to avoid name collisions when transferring back to the submit machine.
GET_GDX_OUTPUT = TRUE
